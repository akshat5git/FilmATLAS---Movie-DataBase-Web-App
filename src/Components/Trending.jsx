...
export const Trending = () =>  {
    ...
    return (
      Trending.length >0 ? <div className='w-full h-[100vh] bg-[#1F1E24]'>
        <div className='w-full h-[70px] px-3 bg-[#1F1E24] flex justify-between items-center'>
        <div className='text-white text-2xl font-bold p-6 flex items-center ' >
                     <div className='w-[10vw] mr-[-50px]'> <i className="ri-arrow-left-line  cursor-pointer" onClick={() => nav(-1)}></i> &nbsp;Trending</div>
                     <SearchTab category="multi" placeholder="Search Trending Content..."
 />       
                  </div>
         <div className='flex gap-4 w-[400px] h-[40px] ' >
          <Dropdown title={"Category"} options={["all","movie","tv"]} onclick={setCategory} />
          <Dropdown title={"Duration"} options={["day","week"]} onclick={setDuration} />
          </div>
          </div>
          <div id='targetscroll' className=' h-[89vh] w-full overflow-y-auto'>
           {Trending && <InfiniteScroll
